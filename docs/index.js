const noSleep=new NoSleep();const tmpCanvas=document.createElement('canvas');let bgm=new Audio('mp3/bgm.mp3');bgm.volume=0.5;let sound=new Audio();let timerInterval;let vibrateInterval;let duration=0;let startTime;let remainingTime;function loadConfig(){if(localStorage.getItem('darkMode')==1){document.body.dataset.theme='dark';}
...
remainingTime=0;var button=document.getElementById('startButton');button.innerText='スタート';button.onclick=function(){startTimer()};var timerText=document.getElementById('timerText');var timerValue=document.getElementById('timerValue');var t=parseInt(timerValue.value);duration=t*60000;if(isNaN(duration)||duration<0){duration=0;}else if(999<t){timerValue.value=999;duration=999*60000;}
...
function tick(){var timerText=document.getElementById('timerText');var min;remainingTime=startTime+duration-Date.now();if(remainingTime<0){bgm.pause();if(remainingTime<-300000){sound.pause();clearInterval(vibrateInterval);}else{if(sound.src.includes('none.mp3')){if(!vibrateInterval){vibrateInterval=setInterval(function(){navigator.vibrate(400);},400);}}else{sound.play();}}
min=Math.ceil(remainingTime/60000);if(min==0){min='-0';}}else{if(localStorage.getItem('bgm')==1){bgm.loop=true;bgm.play();}
min=Math.floor(remainingTime/60000);}
var sec=Math.floor(Math.abs(remainingTime)%60000/1000);timerText.innerText=min+':'+('0'+sec).slice(-2);resizeFontSize(timerText);}
